 ...
 * @module shell
 */
import { AIApplicationId } from './ai';
import EventEmitter from './src/EventEmitter';
import type { SubOrg } from './appcontext';
...
export interface SolutionOrService {
    adobeBrandType: string;
    agentApplicationId?: AIApplicationId;
    appId: string;
    description: string;
    enabled: boolean;
    ...
    href: string;
    isAgentOrchestratorEnabled: boolean;
...
/**
 * A React-style ref object whose `.current` points to the DOM element the
 * Profile popover should anchor to. Typed loosely so this package doesn't take
 * a hard dependency on React types.
 */
...
export interface ShellApi extends EventEmitter<ShellInfoEvent> {
    ...
    /**
     * Opens the Shell-owned Profile popover, anchored to the element referenced
     * by `triggerRef`. The popover is rendered in the parent Shell document; the
     * runtime reads `triggerRef.current.getBoundingClientRect()` and posts the
     * coordinates over the iframe bridge. The Shell then composes those iframe-
     * local coordinates with the iframe's position in the parent viewport so the
     * popover appears anchored to your element in the visible Shell.
     *
     * ***Example:***
     *
     * ```typescript
     ...
     * // ...
     * <button ref={profileButtonRef} onClick={() => shell.openProfile(profileButtonRef)}>
     *   Profile
     * </button>
     * ```
     * @param triggerRef A React-style ref whose `.current` is the DOM element
     * the popover should anchor to. If `.current` is null at call time, the
     * request is dropped.
     */
    openProfile(triggerRef: TriggerRef): void;
}
