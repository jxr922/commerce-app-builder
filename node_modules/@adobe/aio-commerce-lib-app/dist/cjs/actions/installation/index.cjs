...
openwhisk = require_rolldown_runtime.__toESM(openwhisk, 1);
...
async function createFilesStore(options = {}) {
	...
	const files = await (0, _adobe_aio_lib_files.init)();
...
async function createStateStore(options = {}) {
	...
	const state = await (0, _adobe_aio_lib_state.init)();
...
router.post("/uninstallation", {
	...
	handler: async (req, { logger, rawParams }) => {
		...
		if (!uninstallConfig) return (0, _adobe_aio_commerce_lib_core_responses.internalServerError)("Cannot determine what to uninstall: no recorded installation snapshot and no app config was provided.");
		...
		const initialState = require_management.createInitialUninstallationState({ config: require_validate.validateCommerceAppConfig(uninstallConfig) });
...
async function getInstallationSnapshot() {
	...
	return installSnapshot && require_management.isCompletedState(installSnapshot) && installSnapshot.config ? installSnapshot : null;
}
