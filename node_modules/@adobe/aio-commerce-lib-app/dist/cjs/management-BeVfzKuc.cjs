...
const require_webhooks = require('./webhooks-uYZZmgTg.cjs');
...
function assertScriptModule(module) {
	require_environment.parseOrThrow(ScriptModuleSchema, module, "Invalid script module format. Expected a function or an object with an `install` method.");
}
/**
...
function createAdminUiClient(params) {
	...
	return (0, _adobe_aio_commerce_lib_admin_ui_api.createAdminUiApiClient)(commerceClientParams);
}
...
const adminUiStep = defineBranchStep({
	...
	name: "admin-ui",
	when: require_webhooks.hasAdminUi
});
...
async function createOrGetIoEventProvider(params, existingData) {
	...
	const existing = require_utils.findExistingProvider(existingData, instanceId);
	if (existing) {
...
async function createIoEventProviderEventMetadata(params) {
	...
	const eventCode = require_utils.getIoEventCode(require_utils.getNamespacedEvent(metadata, event.name), type);
...
async function createOrGetIoProviderEventMetadata(params, existingData) {
	...
	const { logger } = context;
	const eventCode = require_utils.getIoEventCode(require_utils.getNamespacedEvent(metadata, event.name), type);
	const existing = require_utils.findExistingProviderMetadata(existingData, eventCode);
	if (existing) {
...
async function createIoEventRegistration(params) {
	...
	const name = require_utils.getRegistrationName(provider, runtimeAction);
	const description = require_utils.getRegistrationDescription(provider, events, runtimeAction);
	const payload = {
		...appCredentials,
		...
		eventsOfInterest: events.map((event) => ({
			eventCode: require_utils.getIoEventCode(require_utils.getNamespacedEvent(metadata, event.name), provider.provider_metadata),
			providerId: provider.id
		})),
		name,
		runtimeAction
...
async function createOrGetIoEventRegistration(params, registrations) {
	...
	const name = require_utils.getRegistrationName(provider, runtimeAction);
	const existing = require_utils.findExistingRegistrations(registrations, runtimeParams.AIO_COMMERCE_AUTH_IMS_CLIENT_ID, name);
	if (existing) {
...
async function configureCommerceEventing(params, existingData) {
	...
	const updateParams = require_utils.getCommerceEventingConfigurationUpdateParams(config, existingData);
...
async function createOrGetCommerceProvider(params, existingProviders) {
	...
	const existingByInstanceId = require_utils.findExistingProvider(existingProviders, provider.instance_id);
...
async function createCommerceEventSubscription(params) {
	...
	const eventName = require_utils.getNamespacedEvent(metadata, event.config.name);
...
async function createOrGetCommerceEventSubscription(params, existingData) {
	...
	const { logger } = context;
	const eventName = require_utils.getNamespacedEvent(metadata, event.config.name);
	const existing = require_utils.findExistingSubscription(existingData, eventName);
	if (existing) {
...
async function onboardIoEvents(params, existingData) {
	...
	const instanceId = require_utils.generateInstanceId(metadata, provider, context.appData.workspaceId);
	...
	const actionEventsMap = require_utils.groupEventsByRuntimeActions(events);
...
async function deleteIoEventRegistrations(providerData, provider, events, registrations, context) {
	...
	const registrationNames = /* @__PURE__ */ new Set([...runtimeActions.map((runtimeAction) => require_utils.getRegistrationName(providerData, runtimeAction)), ...runtimeActions.map((runtimeAction) => require_utils.getLegacyRegistrationName(providerData, runtimeAction))]);
...
async function offboardIoEvents(params, existingData) {
	...
	const instanceId = require_utils.generateInstanceId(metadata, provider, appData.workspaceId);
	const instanceIdOldVersion = require_utils.generateInstanceIdDeprecated(metadata, provider);
...
async function deleteCommerceEventSubscriptions(events, metadata, provider, existingSubscriptions, context) {
	...
	for (const event of events) {
		const eventName = require_utils.getNamespacedEvent(metadata, event.name);
...
async function createCommerceEvents(config, context) {
	...
	const stepData = [];
	const env = require_environment.getInstallCommerceEnv(context.params);
	const workspaceConfiguration = JSON.stringify(require_utils.makeWorkspaceConfig(context));
	const existingIoEventsData = await require_utils.getIoEventsExistingData(context);
	const commerceEventingExistingData = await require_utils.getCommerceEventingExistingData(context);
	let eventingConfigured = false;
	const storedProviders = {};
	for (const { provider, events: providerEvents } of config.eventing.commerce) {
		const events = providerEvents.filter((event) => require_environment.appliesToEnv(event, env));
		if (events.length === 0) {
		...
		const { providerData, eventsData } = await onboardIoEvents({
			...
			provider,
			providerType: require_utils.COMMERCE_PROVIDER_TYPE
		}, existingIoEventsData);
		if (!eventingConfigured) {
			await configureCommerceEventing({
				config: {
					enabled: true,
					environment_id: require_utils.sanitizeEventingIdentifier(context.appData.projectName),
					instance_id: providerData.instance_id,
					merchant_id: require_utils.sanitizeEventingIdentifier(context.appData.orgName),
					workspace_configuration: workspaceConfiguration
				},
				context
	...
	const existing = await (0, _adobe_aio_commerce_lib_config.getSystemConfigByKey)("events") ?? { providers: {} };
	await (0, _adobe_aio_commerce_lib_config.setSystemConfigByKey)(require_utils.EVENTS_STORAGE_KEY, { providers: {
		...existing.providers,
		...storedProviders
	} });
...
async function removeCommerceEvents(config, context) {
	...
	const [existingIoEventsData, commerceEventingExistingData] = await Promise.all([require_utils.getIoEventsExistingData(context), require_utils.getCommerceEventingExistingData(context)]);
...
async function createExternalEvents(config, context) {
	...
	const eligibleProviders = config.eventing.external.map(({ provider, events: providerEvents }) => ({
		events: providerEvents.filter((event) => require_environment.appliesToEnv(event, env)),
		provider
	...
	const stepData = await Promise.all(eligibleProviders.map(async ({ provider, events }) => {
		const { providerData, eventsData } = await onboardIoEvents({
			...
			metadata: config.metadata,
			provider,
			providerType: require_utils.EXTERNAL_PROVIDER_TYPE
		}, existingIoEventsData);
		if (provider.key) storedProviders[provider.key] = {
			events: Object.fromEntries(eventsData.map(({ config: eventConfig, data: eventData }) => [eventConfig.name, {
				code: eventData.metadata.event_code,
...
async function validateWebhookConflicts(config, context) {
	...
	const env = require_environment.getInstallCommerceEnv(params);
	const modificationWebhooks = config.webhooks.filter((entry) => entry.category === "modification" && require_environment.appliesToEnv(entry, env));
...
async function createWebhookSubscriptions(config, context) {
	const { logger, commerceWebhooksClient, params } = context;
	const env = require_environment.getInstallCommerceEnv(params);
	const webhooks = config.webhooks.filter((entry) => require_environment.appliesToEnv(entry, env));
