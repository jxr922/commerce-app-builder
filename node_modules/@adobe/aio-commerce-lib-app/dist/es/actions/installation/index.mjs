...
import { t as validateCommerceAppConfig } from "../../validate-B4S4D4NW.mjs";
import { a as runValidation, c as isFailedState, i as runUninstallation, l as isInProgressState, n as createInitialUninstallationState, r as runInstallation, s as isCompletedState, t as createInitialInstallationState, u as isSucceededState } from "../../management-CfYDBXqP.mjs";
...
import { init as init$1 } from "@adobe/aio-lib-files";
...
async function createFilesStore(options = {}) {
	...
	const files = await init$1();
	return new FilesStore(files, dirPrefix);
}
/**
...
async function createStateStore(options = {}) {
	...
	return new StateStore(state, keyPrefix, ttlSeconds);
}
/**
...
var CombinedStore = class {
	...
	async delete(key) {
		const [cacheDeleted, persistentDeleted] = await Promise.all([this.cache.delete(key), this.persistent.delete(key)]);
...
router.post("/uninstallation", {
	...
	handler: async (req, { logger, rawParams }) => {
		...
		const installAppConfig = (await getInstallationSnapshot())?.config;
		const uninstallConfig = installAppConfig ?? rawParams.appConfig;
		if (!uninstallConfig) return internalServerError("Cannot determine what to uninstall: no recorded installation snapshot and no app config was provided.");
		logger.debug(installAppConfig ? "Sourcing uninstallation from recorded install snapshot" : "No recorded install config found; falling back to request config");
		const initialState = createInitialUninstallationState({ config: validateCommerceAppConfig(uninstallConfig) });
...
async function getInstallationSnapshot() {
	const installSnapshot = await (await createInstallationStore()).get(getStorageKey());
	return installSnapshot && isCompletedState(installSnapshot) && installSnapshot.config ? installSnapshot : null;
}
...
//#endregion
export { defineCustomScriptsLoader, installationRuntimeAction };